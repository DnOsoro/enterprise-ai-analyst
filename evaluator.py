...
class SQLEvaluator:
    ...
    Supports both PostgreSQL connection objects and DuckDB connection objects.
    """
    def __init__(self, db_connection: Any, engine_type: str = "postgres"):
        """
        :param db_connection: Active DB connection (psycopg2 connection or duckdb connection)
        :param engine_type: 'postgres' or 'duckdb'
        """
    ...
    def execute_query(self, query: str) -> Tuple[bool, Optional[pd.DataFrame], Optional[str], float]:
        """
        Executes a SQL query safely and measures execution time.
        Returns: (success_flag, result_dataframe, error_message, execution_time_seconds)
        """
    ...
    def compare_dataframes(self, df_generated: pd.DataFrame, df_truth: pd.DataFrame) -> bool:
        """
        Checks if the generated query output matches the ground truth output.
        Ignores column naming differences where applicable, but validates shape and content.
        """
    ...
    def evaluate_sample(self, question: str, generated_sql: str, ground_truth_sql: str) -> Dict[str, Any]:
        """
        Evaluates a single generated query against ground truth.
        """
